...
# XGBoost expects labels to be 0 and 1.
# We will map Pass(-1) -> 0 and Fail(1) -> 1.
y_train_xgb = np.where(y_train == 1, 1, 0)
y_test_xgb = np.where(y_test == 1, 1, 0)

# Impute missing values (same as before)
imputer = SimpleImputer(strategy="median")
X_train_imputed = imputer.fit_transform(X_train)
X_test_imputed = imputer.transform(X_test)

# 2. CALCULATE scale_pos_weight FOR IMBALANCE
# This is the key parameter for handling imbalance in XGBoost.
# Formula: count of negative class / count of positive class
num_pass = np.sum(y_train_xgb == 0)
num_fail = np.sum(y_train_xgb == 1)
scale_pos_weight_value = num_pass / num_fail

print(f"Number of 'Pass' samples in training set: {num_pass}")
print(f"Number of 'Fail' samples in training set: {num_fail}")
print(f"Calculated scale_pos_weight: {scale_pos_weight_value:.2f}")


# 3. TRAIN THE XGBOOST MODEL
# We use the calculated scale_pos_weight to train the model
...
model_xgb.fit(X_train_imputed, y_train_xgb)
print("Training complete.")


# 4. FIND THE OPTIMAL THRESHOLD (same process as before)
# Get probabilities for the positive class (Fail=1)
y_probs_xgb = model_xgb.predict_proba(X_test_imputed)[:, 1]

# Calculate precision, recall, and thresholds
# Remember to use the 0/1 encoded y_test_xgb and pos_label=1
precision, recall, thresholds = precision_recall_curve(y_test_xgb, y_probs_xgb, pos_label=1)

# Find the threshold that maximizes the F1 score
f1_scores = 2 * (precision * recall) / (precision + recall + 1e-9) # add epsilon
best_threshold_xgb = thresholds[np.argmax(f1_scores)]

print(f"\nBest threshold based on F1-score: {best_threshold_xgb:.4f}")


# 5. EVALUATE THE FINAL MODEL
# Apply the optimal threshold to get final predictions
y_pred_optimal_xgb = np.where(y_probs_xgb >= best_threshold_xgb, 1, 0)

print("\nClassification Report for XGBoost (using Optimal Threshold):")
# Use the 0/1 labels and target names
print(classification_report(y_test_xgb, y_pred_optimal_xgb, labels=[0, 1], target_names=["Pass", "Fail"], zero_division=0))
```

    Number of 'Pass' samples in training set: 1170
    Number of 'Fail' samples in training set: 83
    Calculated scale_pos_weight: 14.10
    
    Training XGBoost model...
    

    C:\prog\WPy64-31241\python-3.12.4.amd64\Lib\site-packages\xgboost\training.py:183: UserWarning: [12:14:20] WARNING: C:\actions-runner\_work\xgboost\xgboost\src\learner.cc:738: 
    ...
    Cell In[8], line 46
         42 y_probs_xgb = model_xgb.predict_proba(X_test_imputed)[:, 1]
         44 # Calculate precision, recall, and thresholds
         45 # Remember to use the 0/1 encoded y_test_xgb and pos_label=1
    ---> 46 precision, recall, thresholds = precision_recall_curve(y_test_xgb, y_probs_xgb, pos_label=1)
         48 # Find the threshold that maximizes the F1 score
         49 f1_scores = 2 * (precision * recall) / (precision + recall + 1e-9) # add epsilon
